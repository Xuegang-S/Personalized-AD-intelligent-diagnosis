...
class GraphAttentionLayer(nn.Module):
    ...
    def _prepare_attentional_mechanism_input(self, Wh):
        ...
        # Wh1&2.shape (N, 1)
        # e.shape (N, N)
        Wh1 = self.leakyrelu(torch.matmul(Wh, self.a[:self.out_features, :]))  # a也是一个可学习的参数
        Wh2 = self.leakyrelu(torch.matmul(Wh, self.a[self.out_features:, :]))
        # broadcast add
...
model = GATv2(nfeat=x.shape[1], nhid=8, nclass=7, dropout=0.6, alpha=0.2, nheads=8)
output = model(x, adj)
print(output.shape)
